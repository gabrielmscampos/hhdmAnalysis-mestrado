...
Original repository: https://github.com/Dobiasd/frugally-deep/

Code reformatted using black, isort and flake8
"""
...
def gen_test_data(model):
    ...
    def generate_input_data(input_layer):
        ...
        return random_fn(
            size=replace_none_with(
                32, set_shape_idx_0_to_1_if_none(singleton_list_to_value(shape))
            )
        ).astype(np.float32)
...
def transform_cudnn_weights(input_weights, recurrent_weights, n_gates):
    return transform_kernels(
        input_weights, n_gates, transform_input_kernel
...
def get_transform_func(layer):
    ...
    if layer.__class__.__name__ in ["CuDNNGRU", "CuDNNLSTM"]:
        ...
        def input_transform_func(kernels):
            return transform_kernels(kernels, n_gates, transform_input_kernel)

        def recurrent_transform_func(kernels):
            return transform_kernels(kernels, n_gates, transform_recurrent_kernel)
...
def show_bidirectional_layer(layer):
    ...
    (
        forward_input_transform_func,
        forward_recurrent_transform_func,
        forward_bias_transform_func,
    ) = get_transform_func(layer.forward_layer)

    backward_weights = layer.backward_layer.get_weights()
    assert len(backward_weights) == 2 or len(backward_weights) == 3
    (
        backward_input_transform_func,
        backward_recurrent_transform_func,
        backward_bias_transform_func,
    ) = get_transform_func(layer.backward_layer)

    result = {
        "forward_weights": encode_floats(
            forward_input_transform_func(forward_weights[0])
        ),
    ...
    if hasattr(layer, "data_format"):
        if layer_type in [
            ...
            assert (
                ...
                or layer.data_format == "channels_first"
            )
        else:
...
def get_all_weights(model, prefix):
    ...
    for layer in layers:
        ...
        elif layer_type in ["TimeDistributed"] and type(layer.layer).__name__ in [
            "Model",
            "Sequential",
            "Functional",
        ]:
...
def offset_conv2d_eval(depth, padding, x):
    ...
    kernel = K.variable(
        value=np.array([[[[1]] + [[0]] * (depth - 1)]]), dtype="float32"
    )
...
def offset_sep_conv2d_eval(depth, padding, x):
    ...
    pointwise_kernel = K.variable(
        value=np.array([[[[1]] + [[0]] * (depth - 1)]]), dtype="float32"
    )
    return K.separable_conv2d(
        x, depthwise_kernel, pointwise_kernel, strides=(3, 3), padding=padding
    )
...
def convert(in_path, out_path, no_tests=False):
    ...
    write_text_file(
        out_path, json.dumps(json_output, allow_nan=False, indent=2, sort_keys=True)
    )
